...
def get_data(dataset_name,
    ...
    if randomize_features:
        node_features = np.random.rand(node_features.shape[0],
    ...
    new_test_node_set = set(
        random.sample(test_node_set, int(0.1 * n_total_unique_nodes)))
    ...
    observed_edges_mask = np.logical_and(~new_test_source_mask,
                                         ~new_test_destination_mask)
    ...
    train_data = Data(sources[train_mask], destinations[train_mask],
                      timestamps[train_mask], edge_idxs[train_mask],
                      labels[train_mask], source_next_dest_sequence[train_mask])
    ...
    if different_new_nodes_between_val_and_test:
        ...
        new_node_val_mask = np.logical_and(val_mask,
                                           edge_contains_new_val_node_mask)
        new_node_test_mask = np.logical_and(test_mask,
                                            edge_contains_new_test_node_mask)
    ...
    val_data = Data(sources[val_mask], destinations[val_mask],
                    timestamps[val_mask], edge_idxs[val_mask],
                    labels[val_mask], source_next_dest_sequence[val_mask])

    test_data = Data(sources[test_mask], destinations[test_mask],
                     timestamps[test_mask], edge_idxs[test_mask],
                     labels[test_mask], source_next_dest_sequence[val_mask])
    ...
    new_node_val_data = Data(sources[new_node_val_mask],
                             ...
                             labels[new_node_val_mask], source_next_dest_sequence[new_node_val_mask])
    ...
    print(
        ...
        .format(train_data.n_interactions, train_data.n_unique_nodes))
    print(
        "The validation dataset has {} interactions, involving {} different nodes"
        .format(val_data.n_interactions, val_data.n_unique_nodes))
    print("The test dataset has {} interactions, involving {} different nodes".
          format(test_data.n_interactions, test_data.n_unique_nodes))
    print(
        "The new node validation dataset has {} interactions, involving {} different nodes"
        .format(new_node_val_data.n_interactions,
                new_node_val_data.n_unique_nodes))
    print(
        "The new node test dataset has {} interactions, involving {} different nodes"
        .format(new_node_test_data.n_interactions,
                new_node_test_data.n_unique_nodes))
    print(
        "{} nodes were used for the inductive testing, i.e. are never seen during training"
        .format(len(new_test_node_set)))
...
def compute_time_statistics(sources, destinations, timestamps):
    ...
    for k in range(len(sources)):
        ...
        all_timediffs_src.append(c_timestamp -
                                 last_timestamp_sources[source_id])
